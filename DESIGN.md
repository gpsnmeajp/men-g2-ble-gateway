...
**重複送信スキップ**: ページ構造 (text/image コンテナ構成) が一致する場合は、必要な text update だけを送って rebuild/画像再送を避ける。elements・gamma・dither が完全一致する場合は BLE 送信が 0 件になる。構造変化 (切断後、clear 後、surface 変化後) では常に再構築する。
...
flowchart TD
    ...
    H2 -- yes --> H3{"構造一致で text 差分更新だけで済む?"}
    H3 -- yes --> H4["update_text_message を即時送信"]
    H4 --> H5["必要なら last_display_request 更新"]
    H5 --> H6{"text update を送った?"}
    H6 -- yes --> Z
