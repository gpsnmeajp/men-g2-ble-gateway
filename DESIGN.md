...
3. ライセンスおよび NOTICE ファイルは mentraos/LICENSES 配下に置き、実行時モジュールへ混在させない。
...
3. サーバー設定、BLE タイミング設定、UI 関連設定の保持。
...
11. 画像データ反映は逐次送信とし、複数 image 更新を並列に流さない。
...
4. 各画像領域について、必要に応じて変換とタイル分割を行い、逐次に raw image update を送る。
...
**重複送信スキップ**: ページが既に確立されており、かつ elements・gamma・dither が前回送信時と完全に一致する場合は BLE リビルドと画像送信をスキップする。構造変化 (切断後、clear 後) では常に送信する。
...
flowchart TD
    ...
    H --> H1["elements を dict 化して前回 request と比較"]
    H1 --> H2{"page 確立済み、かつ request が完全一致?"}
    H2 -- yes --> H3["rebuild と画像送信をスキップ"]
    H3 --> H4["必要なら last_display_request を更新"]
    H4 --> Z
    H2 -- no --> I["_build_layout_specs()"]
    ...
    N2 --> O{"ENABLE_POST_IMAGE_TEXT_REDRAW && user text あり?"}
    O -- no --> P
...
BLE 再接続を伴う回復処理は、以下が発生した時点でのみ開始する。
...
1. ページ状態フラグをリセットする。
2. 最後に要求された表示状態と目標マイク状態をメモリに保持する。
3. 左右両方へ再接続する。
4. 初期化を再実行する。
...
現在の既定値では `ENABLE_SYSTEM_EXIT_RUNTIME_RESET` は false である。したがって SYSEXIT / abnormal exit は `glasses.touch` イベントとしては配信されるが、page flags の reset、`last_display_request` の消去、startup page 再確立は行わない。
...
flowchart TD
    ...
    G --> H{"page_shutdown が true?"}
    H -- yes --> I["runtime_reset_reason = page_shutdown"]
    H -- no --> J{"system_exit が true?"}
    J -- no --> K{"runtime_reset_reason あり?"}
    J -- yes --> L{"ENABLE_SYSTEM_EXIT_RUNTIME_RESET?"}
    L -- no --> L1["SYSEXIT は event 配信のみ。page state は維持"]
    L1 --> X["処理終了"]
    L -- yes --> L2["runtime_reset_reason = system_exit_reason"]
    L2 --> L3["last_display_request = None"]
    L3 --> K
    I --> K
    K -- no --> X
    K -- yes --> M["page.reset_runtime_flags()"]
    M --> N["mic_enabled = False"]
    N --> O["status.snapshot を発行"]
    O --> P["_restore_runtime_after_page_reset(reason)"]
    P --> Q{"state.ready?"}
    Q -- no --> X
    Q -- yes --> R["system.reinitialize を emit"]
    R --> S["_ensure_startup_page(' ')"]
    S --> T{"last_display_request が残っている?"}
    T -- yes --> U["display(cached_display, remember=False)"]
    T -- no --> V{"target_mic_enabled?"}
    U --> V
    V -- yes --> W["set_mic_enabled(True)"]
    V -- no --> X
    W --> X
```
...
2. MentraOS のライセンス文面は mentraos/LICENSES/MentraOS_LICENSE に複製する。
3. mentraos/LICENSES/NOTICE.md を追加し、プロトコル挙動と構造が MentraOS の G2.kt 実装を元に移植されたことを記載する。
4. 帰属情報ファイルは実行時コードから分離したまま保つ。
