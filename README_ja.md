...
- **差分表示更新** — レイアウト構造が同じ場合はページ再構築を避け、未変更の画像タイルは再送信しない
...
### Bluetooth 利用前の注意と復帰方法

- 利用開始前に、グラスがスマートフォン側と切断されている状態にしてください。スマートフォンの電源を切る、Bluetooth を完全にオフにする、Even アプリや MentraOS を強制終了する、などを行ってください。スマートフォン側と接続されたままだとアドバタイジングが始まらず、ゲートウェイがグラスを検出できないことがあります。
- スマートフォン側のペアリングを解除する必要はありません。
- PC とグラスのペアリング要求画面が表示された場合は、ペアリングを許可してください。許可しないと、一定時間後に通信が切断されることがあります。
- 再度スマートフォンアプリで使いたい場合は、ゲートウェイサーバーを停止し、スマートフォン側の Bluetooth を有効にしてアプリを起動し直してください。場合によっては PC 側の Bluetooth をオフにしたり、PC の電源を一度切る必要があることがあります。
- 動作が不安定なときは、ゲートウェイサーバーの再起動、PC の再起動、グラスの再起動（両耳のタッチパネルを 5 回連続タップ）を試してください。
...
- `ask_character_on_glasses`: ゲーム風対話 UI を話者アイコン（画像、短いテキスト、または絵文字）とスワイプ制御選択肢と共に表示。
- `ask_client_user`: MCP クライアント UI がサポートする場合、構造化入力を要求。

表示ベースのインタラクションツール（`ask_user_on_glasses`、`ask_menu_on_glasses`、`ask_character_on_glasses`）は、選択、キャンセル、またはタイムアウト後に結果を返す前に自動的にグラスの表示をクリアします。
アクティブなグラス接続が必要な MCP ツールは、失敗する前に最大 15 秒間 `glasses.ready` を待機します。
...
現在のページと同じレイアウト構造のリクエストでは、ページ再構築を避けてテキストと画像内容だけを in-place 更新し、未変更の画像タイルは送信しません。
...
単一コンテナ寸法を超える画像は自動でタイル分割されます。同じレイアウト構造の後続リクエストでは、レンダリング後の BMP ペイロードが変わったタイルだけを再送信します。
